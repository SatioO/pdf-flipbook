var PdfFlip = {
    ...
    init: function() {
        ...
        $(document).on(
            "click",
            ".toolbar #previous , .directions .prev-button",
            function(e) {
    ...
    start: function() {
        ...
        $("#viewerContainer").after(
            '<div id="magazineContainer"><div id="magazine"></div></div>'
        );
        $("body").append(
            '<div class="directions"><a href="#" class="prev-button"></a><a href="#" class="next-button"></a></div>'
        );
        ...
        PdfFlip.loadTurnJsPages(pages, $("#magazine"), true, true).then(
            function() {
                $("#magazine").turn({
                    autoCenter: true,
                    display: "single",
                ...
                .then(function(page) {
                    ...
                    setTimeout(() => {
                        page.render(renderContext).promise.then(function() {
                            ...
                            destinationCanvas.setAttribute(
                                "data-page-number",
                                page.pageNumber
                            );
                            destinationCanvas.id =
                                "magCanvas" + page.pageNumber;

                            if (!isInit) {
                                if (
                                    $(magazine).turn("hasPage", page.pageNumber)
                                ) {
                                    var oldCanvas = $(
                                        "#magCanvas" + page.pageNumber
                                    )[0];
